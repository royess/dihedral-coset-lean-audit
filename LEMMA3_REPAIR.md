...
at the square-root scale; omitted covariance and exact-signal terms remain
open.  The exact `{z,-z}` raw second-moment block makes the correction
indefinite, so direct PSD promotion fails before orientation quotienting; the
radial exact problem remains open.
...
only `N^(-0.108737...+o(1))` to any bounded predictor.  Positive-coefficient
